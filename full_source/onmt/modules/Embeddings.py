...
class ComplexPositionEncoding(nn.Module):
    def __init__(self, dropout, dim, max_len=5000):
        weight = torch.empty(max_len, dim)
    ...
    def forward(self, emb):
        ...
        emb = emb * math.sqrt(self.dim)
        emb_real = emb.mul(self.co[:emb.size(0)].unsqueeze(1).expand(out_length, out_batch, emb_size))
        emb_imag = emb.mul(self.sin[:emb.size(0)].unsqueeze(1).expand(out_length, out_batch, emb_size))
        emb = torch.cat((emb_real, emb_imag), -1)
        return emb
...
class Embeddings(nn.Module):
    ...
    def __init__(self, word_vec_size,
        ...
        self.complex_encoding = complex_encoding
        ...
        if complex_encoding:
            emb_complex = ComplexPositionEncoding(dropout, self.embedding_size)
            self.make_embedding.add_module('emb_complex', emb_complex)
