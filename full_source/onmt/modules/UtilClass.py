...
class LayerNorm(nn.Module):
    def __init__(self, features, eps=1e-6):
        ...
        self.a_3 = nn.Parameter(torch.ones(features * 2))
        self.b_3 = nn.Parameter(torch.zeros(features * 2))
        self.eps = eps

    def forward(self, x, Complex=False):
        if Complex:
            emb_size = int(x.size(2)/2)
            x_real = x[:, :, :emb_size]
            x_img = x[:, :, emb_size:]
            mean_real = x_real.mean(-1, keepdim=True)
            std_real = x.std(-1, keepdim=True)
            mean_imag = x_img.mean(-1, keepdim=True)
            std_imag = x_img.std(-1, keepdim=True)
            mean = torch.cat((mean_real, mean_imag), 2)
            std = torch.cat((std_real, std_imag), 2)
            result = self.a_3 * (x - mean) / (std + self.eps) + self.b_3
        else:
