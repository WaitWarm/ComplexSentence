...
def make_context(opt, tgt_dict):
    ...
    if opt.context_type == "HAN_join":
        return nn.ModuleList([HierarchicalContext(opt.rnn_size, opt.dropout, context_type=opt.context_type,
                                                  context_size=opt.context_size, padding_idx=padding_idx, tok_idx=tok),
                              HierarchicalContext(opt.rnn_size, opt.dropout, context_type=opt.context_type,
                                                  context_size=opt.context_size, padding_idx=padding_idx, tok_idx=tok)])
    else:
        ...
        if train_part == "context":
            ...
            if 'join' in model_opt.context_type:
                ...
                pretrained_dict = {k: v for k, v in checkpoint['model'].items() if
                                   k in model_dict and 'doc_context' not in k}
